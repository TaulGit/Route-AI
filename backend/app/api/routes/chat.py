...
async def send_message(request: ChatMessage, current_user: SupabaseUser = Depends(get_current_user)):
    ...
    print(f"[Chat] User ID: {current_user.id}")
    print(f"[Chat] LLM Provider: {llm_provider}")
    print(f"{'='*60}")

    memory_manager = get_memory_manager()
    memory = memory_manager.get_memory(session_id, user_id=current_user.id)
...
async def get_chat_history(session_id: str, current_user: SupabaseUser = Depends(get_current_user)):
    """获取对话历史"""
    memory_manager = get_memory_manager()
    memory = memory_manager.get_memory(session_id, user_id=current_user.id)
...
async def clear_chat_history(session_id: str, current_user: SupabaseUser = Depends(get_current_user)):
    """清空对话历史"""
    memory_manager = get_memory_manager()
